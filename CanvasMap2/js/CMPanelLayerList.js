...
CMPanelLayerList.prototype.AddLayerToList=function(TheElement,LayerIndex,Left,LayerInListTop,TheScene)
{
	...
	TheCheckBox.TheLayer=TheScene.Layers[LayerIndex];
	TheCheckBox.checked=TheScene.Layers[LayerIndex].GetVisible(); // check if the layer is currently visible
	...
	var TheIcon=TheScene.Layers[LayerIndex].GetSetting("IconImage","TheImage");

	if (TheIcon==null)
	{
		var TheIcon=document.createElement('div');
		TheIcon.className="CM_LayerListIconClass";
		TheIcon.TheLayer=TheScene.Layers[LayerIndex];
	
		var TheStyle=TheScene.Layers[LayerIndex].TheStyle;
	...
	var TheName=TheScene.Layers[LayerIndex].Name;
	if (TheName===null) TheName="Untitled";
	TheLayerName.innerHTML=TheName;
	
	TheLayerName.TheLayer=TheScene.Layers[LayerIndex];
